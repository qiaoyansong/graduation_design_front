...
@Component({
  selector: 'app-deliver-auction-admin',
  templateUrl: './deliver-auction-admin.component.html',
  styleUrls: ['./deliver-auction-admin.component.scss']
})
export class DeliverAuctionAdminComponent implements OnInit {
  ...
  public getData(): void {
    ...
    this.adminService.getExchangeAuctionList(condition).subscribe(data => {
  ...
  * 根据对应的id进行发货
  */
  public deliverAuction(id: any): void {
    this.adminService.deliverAuctionById(id).subscribe(data => {
      if (data.code === StatusCode.SUCCESS) {
