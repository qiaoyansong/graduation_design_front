...
import { commoditySummaryValidator, newsSourceValidator, newsTitleValidator } from 'src/app/validator/bussinessValidator';
...
export class SeeAuctionComponent implements OnInit {
  ...
  public getData(): void {
    ...
    this.adminService.getAuction(condition).subscribe(data => {
  ...
  public deleteNewsById(id: any): void {
    this.adminService.deleteAuctionById(id).subscribe(data => {
      if (data.code === StatusCode.SUCCESS) {
        // 成功
